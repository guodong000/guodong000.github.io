---
title: 一文搞懂 macOS 下环境配置
---

首先明确一点：`/etc/profile` `/etc/bashrc` `~/.bash_profile` `~/.profile` `~/.bashrc` 这些文件是和某中 Shell 相关的（比如 bash、sh），当然也存在不使用这些文件的 Shell（比如 zsh）。
...
macOS 下使用 bash 时，通常只需通过 `~/.bashrc` 来配置环境变量即可。
